...
| Microphones | Partial | qcom,wcd9380 over SoundWire into the LPASS TX macro. Both built-in microphones record in stereo on `hw:0,1`: the bottom mic (AMIC1) is the left channel, the top mic (AMIC5) the right. The device tree, config and q6afe boot race fixes are on branch 7.1.7/Alioth-FixUP. `audio-xiaomi-alioth` adds a UCM Mic device. Recording through PipeWire is not tested yet. |
